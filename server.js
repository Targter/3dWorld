...
const io = new Server(server, {
    ...
    pingTimeout: 60000, // Increase timeout for ngrok
    pingInterval: 25000,
});
...
chatNameSpace.on("connection", (socket) => {
    ...
    socket.on("setName", (name) => {
        if (socket.userData && name && name.trim() !== "") {
            socket.userData.name = name.trim();
            console.log(`Chat: Player ${socket.id} set name to: ${socket.userData.name}`);
        }
    });

    socket.on("send-message", (message, time) => {
        // Only send message if user has set a name
        if (socket.userData && socket.userData.name && socket.userData.name.trim() !== "") {
...
// Broadcast player data to all connected clients every 20ms
setInterval(() => {
    const playerData = [];
    ...
    // Broadcast to all connected clients if there are any players
...
updateNameSpace.on("connection", (socket) => {
    ...
    socket.on("setAvatar", (avatarSkin) => {
        if (socket.userData) {
            ...
            console.log(`Player ${socket.id} set avatar to: ${avatarSkin}`);
        }
        // Broadcast to all clients that this player has selected an avatar
